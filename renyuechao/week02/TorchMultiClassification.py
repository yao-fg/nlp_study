...
class TorchModel(nn.Module):
    def __init__(self, input_size):
        ...
        self.linear = nn.Linear(input_size, 5)  # 输出5个类别的得分
        self.loss = nn.CrossEntropyLoss()  # 交叉熵损失

    # 当输入真实标签，返回loss值；无真实标签，返回预测值
    def forward(self, x, y=None):
        x = self.linear(x)  # (batch_size, input_size) -> (batch_size, class_num)
        if y is not None:
            return self.loss(x, y)  # CrossEntropyLoss直接接收类别得分和类别编号
        else:
            return torch.softmax(x, dim=1)  # 预测时转成概率分布，便于观察结果
...
def predict(model_path, input_vec):
    ...
    for vec, res, label in zip(input_vec, result, pred_class):
        print(
            "输入：%s, 预测类别：%d, 各类别概率：%s" % (vec, int(label), res.tolist())
        )  # 打印结果
