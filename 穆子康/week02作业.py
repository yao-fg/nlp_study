...
规律：x是一个5维向量，按照下角标统计，那个数字越大就是几分类。
...
class TorchModel(nn.Module):
    def __init__(self, input_size):
        ...
        self.linear = nn.Linear(input_size, 5)  # 线性层，多分类需要输出一个5维向量
        #self.activation = torch.softmax() #多分类情况，使用softmax归一化函数，交叉熵中自带softmax
        self.loss = nn.CrossEntropyLoss() #计算loss使用交叉熵函数
...
# 随机生成一个5维向量，获取最大数字的角标，按照角标输出一个预期分类结果。
def build_sample():
    ...
    with torch.no_grad():
        ...
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            if np.argmax(y_p) == y_t:
                correct += 1  # 样本分类判断正确
                #idx = np.argmax(y_p)
                #print(f'预测值:{idx},真实值:{y_t},预测值概率:{y_p}')
            else:
                wrong += 1
